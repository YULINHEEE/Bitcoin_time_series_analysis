...
print("Calculating BIC values for ARIMA(p,1,q) models...")
for p in range(max_p + 1):
    for q in range(max_q + 1):
        try:
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                model = ARIMA(bitcoin_log, order=(p, 1, q)).fit()
            bic_matrix.iloc[p, q] = model.bic
            print(f"ARIMA({p},1,{q}): BIC = {model.bic:.2f}")
        except:
...
print(f"Total candidate models identified: {final_candidates}")
